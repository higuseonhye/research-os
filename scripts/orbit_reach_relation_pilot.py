...
from wm_expansion.cell import CellSpec, run_cell as run_episode  # noqa: E402
...
def run_cell(env: Any, args: argparse.Namespace) -> dict[str, Any]:
    """One episode. Everything that decides anything lives in wm_expansion.cell.

    What is left here is what genuinely needs Isaac: reset the environment, read
    the scene, and a three-line callback that writes the command, steps physics
    and reports termination. That callback is the only code in this file the
    tests cannot reach.
    """
    ...
    # What is actually in this scene? It decides whether real contact can reuse
    # an existing rigid body or has to add one, and cannot be determined without
    # a GPU. The v5 sweep answered it: there are no rigid objects at all.
    ...
    def drive(target: np.ndarray) -> bool:
        command[0, :3] = torch.as_tensor(
            target, device=command.device, dtype=command.dtype
        )
    ...
    record = run_episode(
        ...
        CellSpec(
            ...
            slide_damping=args.slide_damping,
        ),
        drive,
    )
    record["scene_inventory"] = scene_inventory
    return record
